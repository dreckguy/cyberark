...
export DB_PRIVATE_IP=$(terraform output db_private_ip| tr -d " "\")
export DB_PUBLIC_IP=$(terraform output db_public_ip| tr -d " "\")
export LB_DNS=$(terraform output lb_dns| tr -d " "\")
echo "starting db"
ssh -i key.pem -o "StrictHostKeyChecking no" ubuntu@$DB_PUBLIC_IP "docker run -p 8529:8529 -e ARANGO_NO_AUTH=1 -d arangodb"
